...
exports.getConversations = https_1.https.onCall({ region: 'us-central1' }, (request) => {
...
exports.helloWorld = https_1.https.onCall({ region: 'asia-east2' }, (request) => {
    console.log('helloWorld function was called');
    return {
        message: 'Hello from asia-east2!',
    };
});
exports.onNewMessage = firestore_1.firestore.onDocumentCreated({
...
exports.createNotificationOnUpdate = firestore_1.firestore.onDocumentUpdated({ document: '{collectionId}/{docId}', region: 'asia-east2' }, async (event) => {
    var _a, _b, _c;
    ...
    if (collectionId === 'products') {
        ...
        if (newLikerId) {
            ...
            const likerName = ((_c = likerSnap.data()) === null || _c === void 0 ? void 0 : _c.displayName) || 'Someone';
...
exports.onNewReview = firestore_1.firestore.onDocumentCreated({ document: 'reviews/{reviewId}', region: 'asia-east2' }, async (event) => {
    const snapshot = event.data;
    if (!snapshot) {
...
exports.onConversationUpdate = firestore_1.firestore.onDocumentUpdated({ document: 'conversations/{conversationId}', region: 'asia-east2' }, async (event) => {
    var _a;
    const { conversationId } = event.params;
